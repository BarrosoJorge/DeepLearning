...
def fit(model, train_loader, val_loader, criterion, optimizer, device, num_epochs, 
    ...
    for epoch in range(num_epochs):
        ...
        if scheduler is not None:
            scheduler.step(val_loss) #actualizar el learning rate según la métrica de validación
        ...
        print(f'Epoch [{epoch + 1}/{num_epochs}], Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}', f'Learning Rate: {current_lr:.2e}')  
